...
def connect_provider(backend):
    ...
    #---didata entry----
        ...
        conn = driver(backend.apikey, backend.apisecret, backend.region)
    #-------------------
...
def get_machine_actions(machine_from_api, conn, extra):
    ...
    if conn.type in (Provider.RACKSPACE_FIRST_GEN, Provider.LINODE,
                     Provider.NEPHOSCALE, Provider.SOFTLAYER, Porivder.DIMENSIONDATA,
...
def create_machine(user, backend_id, key_id, machine_name, location_id,
    ...
    if conn.type is Provider.DOCKER:
        ...
        node = _create_machine_rackspace(conn, public_key, machine_name, image,
                                         size, location)
    #---didata----
    elif conn.type == Provider.DIMENSIONDATA:
        node = _create_machine_dimensiondata(conn, public_key, machine_name, image, network)
    #-------------
...
#---didata---
def _create_machine_dimensiondata(conn, public_key, machine_name, image, network):
    key = public_key.replace('\n', '')
    auth = NodeAuthPassword(pubkey=key)
    try:
        node = conn.create_node(name=machine_name, image=image, auth=auth, ex_description='', ex_network=network)
    except Exception as e:
        raise MachineCreationError("Dimension Data, got exception %s" % e, e)
    return node
#------------
